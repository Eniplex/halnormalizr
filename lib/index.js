...
function visitObject(obj, schema, bag, options) {
  ...
  if (obj.hasOwnProperty("embedded")) {
    var embedded = obj.embedded;
    for (var key in embedded) {
      if (embedded.hasOwnProperty(key)) {
        var entity = visit(embedded[key], schema[key], bag, options);
        assignEntity.call(null, normalized, key, entity);
      }
    }
  }
...
function normalize(obj, schema) {
  ...
  if (schema instanceof _IterableSchema2['default'] && obj.hasOwnProperty("embedded")) {
    var key = schema.getItemSchema().getKey();
